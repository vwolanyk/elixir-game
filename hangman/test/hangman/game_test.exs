defmodule Hangman.GameTest do
  ...
  test "assert new game initialized" do
    {:ok, new_game } = Game.init(Dictionary.random_word())
  ...
  test "game won or lost " do
    for state <- [:won, :lost] do
      {:ok, game} = Game.init("fresh")
      updated_game = game |> Map.put(:game_state, state)
      assert { updated_game, _ } = Game.make_move(updated_game, "y")
    end
  end
  #
